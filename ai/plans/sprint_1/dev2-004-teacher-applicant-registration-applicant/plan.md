...
4. Test suites: logic tests, service tests, GraphQL integration tests (via `setupTestServerLifecycle` + `testClient`), component tests — all green (`bun run test:db`, `bun run test:services`, `bun run test:graphql`, `bun run test:ui:components` as applicable), 100% statement/branch on new logic, `bun run scripts/run-test/run-test.ts` used for DB-bound tests.
